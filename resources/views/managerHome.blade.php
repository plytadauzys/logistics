...
@if(session()->has('user'))
    @foreach($settings as $s)
        <p>{{$s->id - 1}} {{$s->type}} {{$s->description}} {{$s->value}}</p>
    @endforeach
<div id="danger" hidden>
    <div class="alert alert-danger d-flex align-items-center" role="alert">
        ...
        <div>
            ...
            @foreach($data as $d)
                @if($d->state === 'order' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[0]->value && \Carbon\Carbon::now()->diffInDays($d->date))
                ...
                @elseif($d->state === 'contact' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[1]->value && \Carbon\Carbon::now()->diffInDays($d->date))
                    <p>Eksp. {{$d->order_no}}: Nerastas transportas
                        ...
                        @endif
                        <a href="expeditions/{{$d->order_no}}"><button type="button" class="btn btn-outline-danger">Tvarkyti</button></a></p>
                @elseif($d->state === 'received' && \Carbon\Carbon::now()->diffInDays($d->date) > $settings[2]->value && \Carbon\Carbon::now()->diffInDays($d->date))
                    <p>Eksp. {{$d->order_no}}: Ekspedicija neuždaryta daugiau nei 4 dienos.
                        <a href="expeditions/{{$d->order_no}}"><button type="button" class="btn btn-outline-danger">Tvarkyti</button></a></p>

                @elseif($d->state == 'transport' && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[0]) >= $settings[3]->value && \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[0])
                ...
                @elseif($d->state === 'exporting' && $d->progress != count(explode('!!',$d->dates)) && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[$d->progress]) >= $settings[4]->value &&
                    ...
                    @php(session()->pull('ordDanger'))
                    <p hidden>{{session()->push('ordDanger',collect()->push($d->order_no)) }}</p>
                @endif
            @endforeach
            @foreach($client as $c)
        ...
        <div>
            ...
            @foreach($data as $d)
                @if($d->state === 'order' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[5]->value && \Carbon\Carbon::now()->diffInDays($d->date) < $settings[0]->value  &&
                    \Carbon\Carbon::now()->toDateString() > $d->date && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
                    <p>Eksp. {{$d->order_no}}: Nekontaktuota su tiekėju
                        @if(\Carbon\Carbon::now()->diffInDays($d->date) > 3 && \Carbon\Carbon::now()->diffInDays($d->date) < 10)
                            {{\Carbon\Carbon::now()->diffInDays($d->date)}} dienos.
                        @elseif(\Carbon\Carbon::now()->diffInDays($d->date) > 9 && \Carbon\Carbon::now()->diffInDays($d->date) < 21)
                ...
                @elseif($d->state === 'received' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[7]->value && \Carbon\Carbon::now()->diffInDays($d->date) < $settings[2]->value &&
                    ...
                    <p>Eksp. {{$d->order_no}}: Ekspedicija neuždaryta daugiau nei 1 diena.
                        <a href="expeditions/{{$d->order_no}}"><button type="button" class="btn btn-outline-danger">Tvarkyti</button></a></p>
                @elseif($d->state == 'transport' && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[0]) >= $settings[8]->value && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[0]) < $settings[3]->value && \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[0])
                    <p>Eksp. {{$d->order_no}}: Nepradėtas pirmas pasikrovimas adresu {{explode('!!',$d->addresses)[0]}}, planuota
                        pasikrovimo data {{explode('!!',$d->dates)[0]}}
                        <a href="expeditions/{{$d->order_no}}"><button type="button" class="btn btn-outline-danger">Tvarkyti</button></a>
                    </p>
                @elseif($d->state === 'exporting' && $d->progress != count(explode('!!',$d->dates)) && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[$d->progress]) >= $settings[9]->value &&
                        \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[$d->progress] && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[$d->progress]) < $settings[4]->value)
                    <p>Eksp. {{$d->order_no}}: Nepradėtas pasikrovimas adresu {{explode('!!',$d->addresses)[$d->progress]}},
                        planuota pasikrovimo data {{explode('!!',$d->dates)[$d->progress]}}
                        <a href="expeditions/{{$d->order_no}}"><button type="button" class="btn btn-outline-danger">Tvarkyti</button></a>
                    </p>
...
<script>
    ...
    @foreach($data as $d)
        @if($d->state === 'order' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[0]->value && \Carbon\Carbon::now()->toDateString() > $d->date && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
            document.getElementById('danger').hidden = false;
        @elseif($d->state === 'order' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[5]->value &&
        \Carbon\Carbon::now()->diffInDays($d->date) < $settings[0]->value && \Carbon\Carbon::now()->toDateString() > $d->date && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
            document.getElementById('warning').hidden = false;
       //------------------------------------------------------------------------------------------------------------------------------------------------
        @elseif($d->state === 'contact' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[1]->value && \Carbon\Carbon::now()->toDateString() > $d->date && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
            document.getElementById('danger').hidden = false;
        @elseif($d->state === 'contact' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[6]->value &&
        \Carbon\Carbon::now()->diffInDays($d->date) < $settings[1]->value && \Carbon\Carbon::now()->toDateString() > $d->date && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
            document.getElementById('warning').hidden = false;
        //------------------------------------------------------------------------------------------------------------------------------------------------
        @elseif($d->state === 'transport' && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[0]) >= $settings[3]->value && \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[0])
            document.getElementById('danger').hidden = false;
        @elseif($d->state === 'transport' && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[0]) >= $settings[8]->value && \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[0] && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[0]) < $settings[3]->value)
            document.getElementById('warning').hidden = false;
        //------------------------------------------------------------------------------------------------------------------------------------------------
        @elseif($d->state === 'exporting' && $d->progress != count(explode('!!',$d->dates)) && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[$d->progress]) >= $settings[4]->value && \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[$d->progress])
            document.getElementById('danger').hidden = false;
        @elseif($d->state === 'exporting' && $d->progress != count(explode('!!',$d->dates)) && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[$d->progress]) >= $settings[9]->value && \Carbon\Carbon::now()->toDateString() > explode('!!',$d->dates)[$d->progress] && \Carbon\Carbon::now()->diffInDays(explode('!!',$d->dates)[$d->progress]) < $settings[4]->value)
            document.getElementById('warning').hidden = false;
        //------------------------------------------------------------------------------------------------------------------------------------------------
    @elseif($d->state === 'received' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[2]->value && \Carbon\Carbon::now()->toDateString() > $d->date && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
            document.getElementById('danger').hidden = false;
        @elseif($d->state === 'received' && \Carbon\Carbon::now()->diffInDays($d->date) >= $settings[7]->value &&
            \Carbon\Carbon::now()->diffInDays($d->date) < $settings[2]->value && \Carbon\Carbon::now()->toDateString() > $d->dates && \Carbon\Carbon::now()->diffInDays($d->date) > 0)
            document.getElementById('warning').hidden = false;
        @endif
    @endforeach
